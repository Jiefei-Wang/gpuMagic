# 1.Rename the first function argument and add the looped variable in a code format 2.special treatment for the break
# and next
...
# Level 1 compiler Functions: 1.simplify the R code, each line should only have one function call, If not, a temporary
# variable will be created to replace it. 2.If the code only has a symbol and the symbol is not recognized, it will be
# removed
RParser1 <- function(codeMetaInfo) {
    codeMetaInfo1 = parserFrame(RLevel1_parserFunc, RLevel1_checkFunc, RLevel1_updateFunc, codeMetaInfo)
    codeMetaInfo1
}
...
RLevel1_parserFunc <- function(level, codeMetaInfo, curExp) {
    ...
    if (substr(code_char, 1, nchar(GPUVar$openclCode)) == GPUVar$openclCode || substr(code_char, 1, nchar(GPUVar$openclFuncCall)) == 
        GPUVar$openclFuncCall) {
        result$Exp = curExp
        return(result)
    }
