#' @include pkgFunc.R
#' @include hash.R
# ======================GPU resources manager==================== The gpu manager is not supposed to be called by the
# user It manage the all resources on a device The data will be automatically released when the it is not in use
.gpuResourcesManager <- local({
    ...
    list(upload = function(deviceId, data, type) {
        ...
        gpuAd = .Call(C_upload, curDevice[1], curDevice[2], data, as.double(length(data)), getTypeNum(type))
        ...
        gpuAd = .Call(C_gpuMalloc, curDevice[1], curDevice[2], as.double(len), getTypeNum(type))
        ...
        internalVars$memoryUsage[[devKey]] = internalVars$memoryUsage[[devKey]] - internalVars$addressSizeList[[devKey]][[adKey]]
...
# A tiny function that can make the output more compact Auto convert the unit between byte, kb, mb, and gb The input is
...
# This function query the device info and storage the results into the global variable If initialOnly=TRUE the function
# will only update the deviceInfo in the first call
...
selectDevice = function(devices) {
    ...
    for (i in seq_len(length(devices))) {
        ...
        .gpuResourcesManager$globalVars$curDevice[[key]] = as.integer(curDeviceInfo[i, c("platform", "device"), drop = FALSE] - 
            1)
...
# Get the platform and device id If checkInitial=TRUE, the function will check if the device has been selected If
# checkInitial=FALSE, the function will just return the device information without check
