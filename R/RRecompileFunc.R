...
recompile_matrixMult <- function(varInfo, curExp) {
    ...
    code = c(code, paste0(left_var, "=gMatrix(nrow=", matrixNrow(right_var1), 
        ",ncol=", matrixNcol(right_var2), ",shared=", left_info$shared, 
        ",location=", left_info$location, ")"), paste0("for(", pvs, "_i1 in 1:nrow(", 
        left_var, ")){"), paste0("for(", pvs, "_i2 in 1:ncol(", left_var, 
        ...
        paste0(pvs, "_loopNum=ceiling(", matrixNcol(right_var1), "/", ps, 
            ")"), paste0(pvs, "_start=1"), paste0(pvs, "_end=1"), paste0(pvs, 
            "_length=0"), paste0("for(", pvs, "_t in 1:", pvs, "_loopNum){"), 
        paste0(pvs, "_start=", pvs, "_end"), paste0(pvs, "_end=", pvs, 
            "_end+", ps), paste0("if(", pvs, "_end>", matrixNcol(right_var1), 
            "){"), paste0(pvs, "_end=", matrixNcol(right_var1), "+1"), 
        "}", paste0(pvs, "_length=", pvs, "_end-", pvs, "_start"), paste0("for(", 
            pvs, "_t1 in 1:", pvs, "_length){"), paste0(pvs, "[", pvs, 
            "_t1]=", matrixInd(right_var1, paste0(pvs, "_i1"), paste0(pvs, 
                "_start+", pvs, "_t1-1"))), "}", paste0("for(", pvs, "_j2 in 1:", 
            matrixNcol(right_var2), "){"), paste0(pvs, "_tmp=0"), paste0("for(", 
            pvs, "_j1 in 1:", pvs, "_length){"), paste0(pvs, "_tmp=", pvs, 
            "_tmp+", pvs, "[", pvs, "_j1]*", matrixInd(right_var2, paste0(pvs, 
                "_start+", pvs, "_j1-1"), paste0(pvs, "_j2"))), "}", paste0(matrixInd(left_var, 
            paste0(pvs, "_i1"), paste0(pvs, "_j2")), "=", matrixInd(left_var, 
            paste0(pvs, "_i1"), paste0(pvs, "_j2")), "+", pvs, "_tmp"), 
        "}", "}", "}")
