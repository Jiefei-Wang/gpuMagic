...
### Use macro
The `gpuSapply` function has an argument `.macroParms` to specify the macro variable. Therefore, value of the macro will be plugged in into the function and is known for the compiler. for example
```{r,eval=FALSE}
...
If you do need a dynamic feature in you code, you can use the reference function to create it. The reference function works like a macro. For example
...
##Sequence variable
For the sequence variable, you can create it without the limitation. As you see in the `dynamic_example2` function, we have implicitly created a sequence variable `1:n`, whose size is unknown. You can use the function `seq` or `:` to create the sequence. the only exception is that you cannot change the value of the sequence variable, because the variable is in a compact format and does not own a memory. If you do want to create a sequence variable in the memory, an alternative way is to create the sequence first and assign it to a new variable, then the compiler will allocate the memory for the new variable
